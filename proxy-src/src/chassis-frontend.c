...
/**
 * SECTION:chassis-frontend
 * @short_description: a collections of common functions used by chassis frontends 
 *
 ...
 */
int
chassis_frontend_init_glib(void) {
 ...
 */
int
chassis_frontend_init_win32(void) {
#ifdef _WIN32
	WSADATA wsaData;
 ...
 * @prg_name: program-name (usually argv[0])
 * @_base_dir: user-supplied basedir 
 *
 * detect the basedir
 *
 ...
 */
static int
chassis_frontend_lua_setenv(const char *key, const char *value) {
	int r;
#if _WIN32
 ...
 *
 * Returns: (transfer full): a path in the form $basedir/lib/$prg_name/lua/?.lua
 */
char *
chassis_frontend_get_default_lua_path(const char *base_dir, const char *prg_name) {
	return g_build_filename(base_dir, "lib", prg_name, "lua", "?.lua", NULL);
}

/**
 * chassis_frontend_get_default_lua_cpath:
 * @base_dir: base directory
 * @prg_name: program name
 *
 * get the default value for LUA_PATH
 *
 * each OS has its own way of declaring a shared-lib extension (.dll, .dylib, .so, ...)
 * and has its own place for modules (bin/ on windows, lib/$prg_name/lua/)
 */
char *
chassis_frontend_get_default_lua_cpath(const char *base_dir, const char *prg_name) {
 ...
 * @set_path:      if not NULL, set that path
 * @base_dir:      base directory to prepend to the sub_names
 * @subdir_names:  list of sub directories to add to the path
 * @is_lua_path:   TRUE if LUA_PATH should be set, FALSE if LUA_CPATH
 *
 ...
 */
static int
chassis_frontend_init_lua_paths(const char *set_path,
 ...
 * @set_path:      if not NULL, set that path
 * @base_dir:      base directory to prepend to the sub_names
 * @lua_subdirs:   list of sub directories to add to the path
 *
 * set the LUA_PATH 
 *
 * if 'set_path' is not-NULL, we setenv() that path
 ...
 */
int
chassis_frontend_init_lua_path(const char *set_path, const char *base_dir, char **lua_subdirs) {
 ...
 * set the LUA_CPATH 
 *
 * if @set_path is non-NULL, we setenv() that path
 * otherwise we construct a value based on the sub_names and the basedir if the env-var is
 * not set yet
 */
int
chassis_frontend_init_lua_cpath(const char *set_path, const char *base_dir, char **lua_subdirs) {
	return chassis_frontend_init_lua_paths(set_path, base_dir, lua_subdirs, FALSE);
}

/**
 * chassis_frontend_init_plugin_dir:
 * @_plugin_dir: location of the plugin_dir
 * @base_dir: base directory
 *
 * if @_plugin_dir points to a %NULL the plugin-dir is build from the @base_dir
 * and set to @_plugin_dir
 *
 * - on windows the default is $basedir/bin/ 
 * - on unix it is $basedir/lib/PACKAGE/plugins/
 */
int
 ...
 * @plugins:      empty array
 * @plugin_dir:   directory to load the plugins from
 * @plugin_names: NULL terminated list of plugin names
 *
 * load the plugins
 *
 ...
 * @plugins:        array of chassis_plugin structs
 * @option_ctx:     a fresh GOptionContext
 * @argc_p:         pointer to the number of args in argv_p
 * @argv_p:         pointer to arguments to parse
 * @keyfile:        the configfile 
 * @keyfile_section_name: group name in the configfile to pull options from
 * @base_dir:       base directory
 * @gerr: (allow-none): a pointer to a clean GError * or NULL in case the possible error should be ignored
 *
 ...
 * @argc_p:         pointer to the number of args in argv_p
 * @argv_p:         pointer to arguments to parse
 * @print_version:  pointer to int to set if --version is specified
 * @config_file:    pointer to char * to store if --defaults-file is specified
 * @gerr: (allow-none): a pointer to a clean GError * or NULL in case the possible error should be ignored
 *
 ...
 * @opts: options
 * @print_version: (out): destination of the --version value
 * @config_file: (out): destination of the --defaults-file value
 *
...
/**
 * chassis_frontend_print_plugin_versions:
 * @plugins: (element-type chassis_plugin): array of plugins
 *
...
/**
 * chassis_frontend_log_plugin_versions:
 * @plugins: (element-type chassis_plugin): array of plugins
 *
