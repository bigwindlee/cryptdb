...
/**
 * SECTION:chassis-keyfile
 * @short_description: map command line options to configfiles
 *
 * most options on the command line have a direct corresping in option in
 * a configfile.
 */
 ...
 * @keyfile: the keyfile
 * @ini_group_name: group name in the keyfile
 * @config_entries: config-entries
 *
 * map options from the keyfile to the config-options
 *
 * Returns: -1 on error, 0 on success
 * Deprecated: 0.8.3: use chassis_keyfile_to_options_with_error() instead
 */
int
chassis_keyfile_to_options(GKeyFile *keyfile, const gchar *ini_group_name, GOptionEntry *config_entries) {
 ...
 * @ini_group_name: group name in the keyfile
 * @config_entries: config-entries
 * @gerr: a #GError if error reporting is wanted, a %NULL otherwise
 *
 * map options from the keyfile to the config-options
 *
 ...
 */
gboolean
chassis_keyfile_to_options_with_error(GKeyFile *keyfile, const gchar *ini_group_name, GOptionEntry *config_entries,
		GError **_gerr) {
	GError *gerr = NULL;
	gboolean ret = TRUE;
	int i, j;
 ...
 */
int
chassis_keyfile_resolve_path(const char *base_dir, GOptionEntry *config_entries) {
	int entry_idx;
