...
/**
 * SECTION:chassis-log
 * @short_description: Hierarchical Logging
 * 
 * # Overview
 * 
 ...
 *
 * A log domain named "chassis.network.backend" is the child of "chassis.network" and so on.
 *
 * If "chassis.network" has, via a configuration file, been assigned the log level MESSAGE, then "chassis.network.backend" would
 * inherit this log level (assuming there was no explicit configuration for it, too).
 *
 * The same is true for the domain's backend - most likely a file name to write to.
 * 
 * There always is a "root domain" with the name of "" (empty string). If no other domain is configured, every other domain will inherit its
 ...
 * single function, namely chassis_log_domain_log().
 */

/**
 * chassis_log_init:
 *
 * create a #chassis_log_t
 *
 * Deprecated: 0.8.0: use chassis_log_new() instead
 */
chassis_log_t *
chassis_log_init(void) {
...
/**
 * chassis_log_level_string_to_level:
 * @level: name of a log-level
 *
 * get the #GLogLevelFlags for a @level
 *
 * Returns: a #GLogLevelFlags or 0 if the @level isn't known
 */
GLogLevelFlags
chassis_log_level_string_to_level(const gchar *level) {
	GQuark level_quark = g_quark_from_string(level);
 ...
 * @log: a #chassis_log_t
 *
 * free a #chassis_log_t
 */
void
chassis_log_free(chassis_log_t *log) {
	if (NULL == log) return;
 ...
 * @log: a #chassis_log_t
 *
 * ask the log to reopen its files asyncronously
 */
void
chassis_log_set_logrotate(chassis_log_t *log) {
 ...
 * @backend: the backend to be added
 *
 * Registers a domain backend so it can be used with individual domains.
 * 
 * Returns: %TRUE if the domain was registered, %FALSE if the registration failed or it already was registered (you should dispose backend yourself in this case)
 */
gboolean
chassis_log_register_backend(chassis_log_t *log, chassis_log_backend_t *backend) {
...
/**
 * chassis_log_register_domain:
 * @log: a log 
 * @domain: a log domain
 *
 * Register a domain
 *
 * Returns: TRUE on success, FALSE if the registration failed or it already was registered (you should dispose backend yourself in this case)
 */
gboolean
chassis_log_register_domain(chassis_log_t *log, chassis_log_domain_t *domain) {
	GHashTable *domains;
...
/**
 * chassis_log_unregister_domain:
 * @log: a #chassis_log_t
 * @domain: a #chassis_log_domain_t
 *
 * unregister @domain from @log
 *
 * note: currently not implemented
 */
void
chassis_log_unregister_domain(chassis_log_t G_GNUC_UNUSED *log, chassis_log_domain_t G_GNUC_UNUSED *domain) {
...
/**
 * chassis_log_get_domain:
 * @log: a #chassis_log_t
 * @domain_name: a domain name
 *
 * get the domain from the @log based on its @domain_name
 *
 * if the @domain_name doesn't exist yet a implicit domain is auto-created, registered and returned.
 *
 * Returns: a #chassis_log_domain_t with the name @domain_name
 */
chassis_log_domain_t *
chassis_log_get_domain(chassis_log_t *log, const gchar *domain_name) {
 ...
 * @log: a #chassis_log_t
 *
 * reopen all backends of @log
 */
void
chassis_log_reopen(chassis_log_t *log) {
 ...
 *
 * change the file ownership with chown() for all backends
 *
 * Returns: TRUE on success, FALSE otherwise
 */
gboolean
chassis_log_chown(chassis_log_t *log, uid_t uid, gid_t gid, GError **gerr) {
	GHashTableIter iterator;
	gpointer key, value;
...
/**
 * chassis_log_force_log_all:
 * @log: a log
 * @message: message to log
 *
 * log a message on all backends
 */
void
chassis_log_force_log_all(chassis_log_t *log, const gchar *message) {
	GHashTableIter iterator;
	gpointer key, value;
...
/**
 * chassis_log_func:
 * @log_domain: the name of the domain this message belongs to.
 * @log_level: the log_level this message is on
 * @message: the pre-formatted message to log
 * @user_data: a pointer to chassis_log
 *
 * Interface to glib2's logging system.
 * 
 * chassis_log_domain_log() looks up the corresponding domain from the log_domain and passes control on.
...
/**
 * chassis_log_get_effective_level_and_target:
 * @log: the extended_log structure
 * @domain_name: name of the domain to get the level for
 * @target: optional output value to also get the effective target of the domain
 *
 * Internal helper function for chassis_log_get_effective_level()
 *
 * Returns: the effective log level for the domain_name
 */
static GLogLevelFlags
chassis_log_get_effective_level_and_target(chassis_log_t *log,
...
/**
 * chassis_log_get_effective_level:
 * @log: a log
 * @domain_name: name of a log-domain
 *
 * get the effective log-level of the @domain_name
 *
 * Returns: a log-level
 */
GLogLevelFlags
chassis_log_get_effective_level(chassis_log_t *log, const gchar *domain_name) {
...
/**
 * chassis_log_extract_hierarchy_names:
 * @domain_name: a hierarchical name
 * @len: pointer where to store the number of names, may to be %NULL
 *
 ...
 *  { "", "a", "a.b", NULL }
 * 
 * Returns: an array of hierarchy names or %NULL if @domain_name was %NULL
 */
gchar**
chassis_log_extract_hierarchy_names(const gchar *domain_name, gsize *len) {
	gchar **substrings;
	gchar *occurrence;
...
/**
 * chassis_log_set_default:
 * @log:           the log
 * @log_filename:  file to log to. If NULL, it logs to stderr
 * @log_lvl:       minimum log-level for the default log
 *
 ...
 */
int
chassis_log_set_default(chassis_log_t *log, const char *log_filename, GLogLevelFlags log_lvl) {
