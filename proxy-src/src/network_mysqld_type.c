...
/**
 * SECTION:network_mysqld_type
 * @short_description: type encoders/decoders
 *
 * decoders for the MySQL types as seen in binary rows
 */
...
/**
 * network_mysqld_type_int_new:
 *
 * create a type that can hold a MYSQL_TYPE_LONGLONG
 */
static network_mysqld_type_int_t *
network_mysqld_type_int_new(void) {
...
/**
 * network_mysqld_type_int_free:
 *
 * free a network_mysqld_type_int_t
 */
static void
network_mysqld_type_int_free(network_mysqld_type_int_t *ll) {
	if (NULL == ll) return;
...
static int
network_mysqld_type_data_int_get_int(network_mysqld_type_t *type, guint64 *i, gboolean *is_unsigned) {
...
static int
network_mysqld_type_data_int_set_int(network_mysqld_type_t *type, guint64 i, gboolean is_unsigned) {
	network_mysqld_type_int_t *value;
 ...
 */
static void
network_mysqld_type_data_int_free(network_mysqld_type_t *type) {
	if (NULL == type) return;
...
static void
network_mysqld_type_data_int_init(network_mysqld_type_t *type, enum enum_field_types field_type) {
...
/**
 * network_mysqld_type_double_new:
 *
 * create a type that can hold a MYSQL_TYPE_DOUBLE
 */
static network_mysqld_type_double_t *
network_mysqld_type_double_new(void) {
...
/**
 * network_mysqld_type_double_free:
 *
 * free a network_mysqld_type_double_t
 */
static void
network_mysqld_type_double_free(network_mysqld_type_double_t *t) {
	if (NULL == t) return;
...
static void
network_mysqld_type_data_double_free(network_mysqld_type_t *type) {
	if (NULL == type) return;
	if (NULL == type->data) return;
...
static int
network_mysqld_type_data_double_get_double(network_mysqld_type_t *type, double *d) {
...
static int
network_mysqld_type_data_double_set_double(network_mysqld_type_t *type, double d) {
...
static void
network_mysqld_type_data_double_init(network_mysqld_type_t *type, enum enum_field_types field_type) {
	type->type	= field_type;
...
/**
 * network_mysqld_type_float_new:
 *
 * create a type that can hold a MYSQL_TYPE_FLOAT
 */
static network_mysqld_type_float_t *
network_mysqld_type_float_new(void) {
...
/**
 * network_mysqld_type_float_free:
 *
 * free a network_mysqld_type_float_t
 */
static void
network_mysqld_type_float_free(network_mysqld_type_float_t *t) {
	if (NULL == t) return;
...
static void
network_mysqld_type_data_float_free(network_mysqld_type_t *type) {
	if (NULL == type) return;
	if (NULL == type->data) return;
...
static int
network_mysqld_type_data_float_get_double(network_mysqld_type_t *type, double *dst) {
...
static int
network_mysqld_type_data_float_set_double(network_mysqld_type_t *type, double src) {
...
static void
network_mysqld_type_data_float_init(network_mysqld_type_t *type, enum enum_field_types field_type) {
	type->type	= field_type;
...
/* MYSQL_TYPE_STRING */
static network_mysqld_type_string_t *
network_mysqld_type_string_new(void) {
...
static void
network_mysqld_type_string_free(network_mysqld_type_string_t *str) {
	if (NULL == str) return;
...
static void
network_mysqld_type_data_string_free(network_mysqld_type_t *type) {
	if (NULL == type) return;
...
static int
network_mysqld_type_data_string_get_string_const(network_mysqld_type_t *type, const char **dst, gsize *dst_len) {
...
static int
network_mysqld_type_data_string_set_string(network_mysqld_type_t *type, const char *src, gsize src_len) {
	GString *dst;
...
static void
network_mysqld_type_data_string_init(network_mysqld_type_t *type, enum enum_field_types field_type) {
	type->type	= field_type;
...
/* MYSQL_TYPE_DATE */
static network_mysqld_type_date_t *
network_mysqld_type_date_new(void) {
...
static void
network_mysqld_type_date_free(network_mysqld_type_date_t *date) {
	if (NULL == date) return;
...
static void
network_mysqld_type_data_date_free(network_mysqld_type_t *type) {
	if (NULL == type) return;
...
static int
network_mysqld_type_data_date_get_date(network_mysqld_type_t *type, network_mysqld_type_date_t *dst) {
...
static gboolean
network_mysqld_type_date_time_is_valid(network_mysqld_type_date_t *date) {
...
static gboolean
network_mysqld_type_date_date_is_valid(network_mysqld_type_date_t *date) {
	return (date->day <= 31 &&
	      date->month <= 12 &&
	      date->year  <= 9999);
}
...
gboolean
network_mysqld_type_date_is_valid(network_mysqld_type_date_t *date) {
	return network_mysqld_type_date_time_is_valid(date) &&
		network_mysqld_type_date_date_is_valid(date);
}

static int
network_mysqld_type_data_date_get_string(network_mysqld_type_t *type, char **dst, gsize *dst_len) {
	network_mysqld_type_date_t *src = type->data;
...
static int
network_mysqld_type_data_date_set_date(network_mysqld_type_t *type, network_mysqld_type_date_t *src) {
...
static void
network_mysqld_type_data_date_init(network_mysqld_type_t *type, enum enum_field_types field_type) {
	type->type	= field_type;
...
/* MYSQL_TYPE_TIME */
static network_mysqld_type_time_t *
network_mysqld_type_time_new(void) {
...
static void
network_mysqld_type_time_free(network_mysqld_type_time_t *t) {
	if (NULL == t) return;
...
static void
network_mysqld_type_data_time_free(network_mysqld_type_t *type) {
	if (NULL == type) return;
...
static int
network_mysqld_type_data_time_get_time(network_mysqld_type_t *type, network_mysqld_type_time_t *dst) {
...
static int
network_mysqld_type_data_time_get_string(network_mysqld_type_t *type, char **dst, gsize *dst_len) {
	network_mysqld_type_time_t *src = type->data;
...
static int
network_mysqld_type_data_time_set_time(network_mysqld_type_t *type, network_mysqld_type_time_t *src) {
	network_mysqld_type_date_t *dst;
...
static void
network_mysqld_type_data_time_init(network_mysqld_type_t *type, enum enum_field_types field_type) {
	type->type	= field_type;
...
/**
 * network_mysqld_type_new:
 * @field_type: type
 *
 * create a type 
 */
network_mysqld_type_t *
network_mysqld_type_new(enum enum_field_types field_type) {
...
/**
 * network_mysqld_type_free:
 * @type: a #network_mysqld_type_t
 *
 * free a type
 */
void
network_mysqld_type_free(network_mysqld_type_t *type) {
	if (NULL == type) return;
...
/**
 * network_mysqld_type_get_gstring:
 * @type: the type to get the data from
 * @s:    GString that the converted data will be assigned too
 *
...
/**
 * network_mysqld_type_get_string_const:
 * @type: the type to get the data from
 * @s:    place to store the pointer to the const char * in
 * @s_len: length of @s
 *
...
/**
 * network_mysqld_type_get_string:
 * @type: the type to get the data from
 * @s:    pointer to a buffer of *s_len size or pointer to (char *)NULL for alloc-mode
 * @s_len: pointer to the length of the buffer if *s is not %NULL. Points to the length of the *s on success
 *
...
/**
 * network_mysqld_type_get_int:
 * @type: a network_mysqld_type_t
 * @i: storage for the value
 * @is_unsigned: will be set to TRUE if @i is unsigned
 *
 * get ->data as (u)int64
 *
 * Returns: 0 on success, -1 on error
...
/**
 * network_mysqld_type_set_int:
 * @type: a #network_mysqld_type_t
 * @i: value
 * @is_unsigned: TRUE if @i is unsigned
 *
 * set ->data from (u)int64
 *
 * Returns: 0 on success, -1 on error
 */
...
/**
 * network_mysqld_type_get_double:
 * @type: a #network_mysqld_type_t
 * @d: value
 *
 * get ->data as double
...
/**
 * network_mysqld_type_set_double:
 * @type: a #network_mysqld_type_t
 * @d: value
 *
...
/**
 * network_mysqld_type_get_date:
 * @type: a #network_mysqld_type_t
 * @date: a #network_mysqld_date_t
 *
 * get the ->data as #network_mysqld_type_date_t 
 *
 * Returns: 0 on success, -1 on error
 */
...
/**
 * network_mysqld_type_set_date:
 * @type: a #network_mysqld_type_t
 * @date: a #network_mysqld_date_t
 *
 * set the ->data from a #network_mysqld_type_date_t
 *
 * Returns: 0 on success, -1 on error
 */
...
/**
 * network_mysqld_type_get_time:
 * @type: a #network_mysqld_type_t
 * @t: a #network_mysqld_time_t
 *
 * get the ->data as #network_mysqld_type_time_t 
 *
 * Returns: 0 on success, -1 on error
 */
...
/**
 * network_mysqld_type_set_time:
 * @type: a #network_mysqld_type_t
 * @t: a #network_mysqld_time_t
 *
 * set the ->data from a #network_mysqld_type_time_t
 *
 * Returns: 0 on success, -1 on error
 */
