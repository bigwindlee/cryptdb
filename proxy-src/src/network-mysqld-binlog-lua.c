...
/**
 * SECTION:network-mysqld-binlog-lua
 * @short_description: Lua wrappers around network-mysqld-binlog
 *
 * Lua wrappers around network-mysqld-binlog
 */
...
/**
 * lua_mysqld_binlog_rows_event_getmetatable:
 * @L: a lua_State
 *
 * push metatable for a #network_mysqld_binlog_event on the @L stack
 */
...
/**
 * lua_mysqld_binlog_rows_event_push:
 * @L: a lua_State
 * @udata: a binlog event
 *
 * wrap and push a binlog event on the @L stack
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_tablemap_event_getmetatable:
 * @L: a lua_State
 *
 * push metatable for a table-map on the @L stack
 */
...
/**
 * lua_mysqld_binlog_tablemap_event_push:
 * @L: a lua_State
 * @udata: a binlog event
 *
 * wrap and push a binlog table-map on the @L stack
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_format_event_getmetatable:
 * @L: a lua_State
 *
 * push metatable for a #network_mysqld_binlog_event on the @L stack
 */
...
/**
 * lua_mysqld_binlog_format_event_push:
 * @L: a lua_State
 * @udata: a binlog event
 *
 * wrap and push a format description event on the @L stack
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_intvar_event_getmetatable:
 * @L: a lua_State
 *
 * push metatable for the intvar part a #network_mysqld_binlog_event on the stack @L
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_intvar_event_push:
 * @L: a lua_State
 * @udata: a binlog event
 *
 * wrap and push the intvar part of the binlog event on the stack @L
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_xid_event_getmetatable:
 * @L: a lua_State
 *
 * push metatable for the xid part a #network_mysqld_binlog_event on the stack @L
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_xid_event_push:
 * @L: a lua_State
 * @udata: a binlog event
 *
 * wrap and push the xid part of the binlog event on the stack @L
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_query_event_getmetatable:
 * @L: a lua_State
 *
 * push metatable for the query part a #network_mysqld_binlog_event on the stack @L
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_query_event_push:
 * @L: a lua_State
 * @udata: a binlog event
 *
 * wrap and push the query part of the binlog event on the stack @L
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_event_gc:
 * @L: a lua_State
 *
 * free a wrapped #network_mysqld_binlog_event
 */
...
/**
 * lua_mysqld_binlog_event_getmetatable:
 * @L: a lua_State
 *
 * push metatable for a #network_mysqld_binlog_event on the stack @L
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_event_push:
 * @L: a lua_State
 * @udata: a binlog event
 * @free_me: %TRUE if the _gc shall free the wrapped event
 *
 * wrap and push the binlog event on the stack @L
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * network_mysqld_binlog_row_event_from_myisam_row:
 * @event: a event
 * @row: a row
 *
 * convert a @event into @row
 *
 * Returns: 0
 */
int network_mysqld_binlog_row_event_from_myisam_row(network_mysqld_binlog_event *event, 
	...
	if (event->event.row_event.used_columns_after_len == 0) {
		event->event.row_event.used_columns_after_len = (int)((event->event.row_event.columns_len+7)/8);
		event->event.row_event.used_columns_after = g_new0(gchar, event->event.row_event.used_columns_after_len);
	}
	...
	for (i = 0; i < row->fields->len; i++) {
		...
		event->event.row_event.used_columns_after[i / 8] |= 1 << (i % 8);
...
/**
 * lua_mysqld_binlog_getmetatable:
 * @L: a lua_State
 *
 * push metatable for a #network_mysqld_binlog on the stack @L
 *
 * Returns: number of items pushed on the stack
 */
...
/**
 * lua_mysqld_binlog_push:
 * @L: a lua_State
 * @udata: #network_mysqld_binlog to wrap
 *
 * wrap a #network_mysqld_binlog and push it on the stack
 */
