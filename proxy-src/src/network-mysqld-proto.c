...
/**
 * SECTION:network-mysqld-proto
 * @short_description: decoders and encoders for the MySQL packets, low level
 *
 * - basic data-types
...
/**
 * network_mysqld_proto_peek_lenenc_type:
 ...
 */
int
network_mysqld_proto_peek_lenenc_type(network_packet *packet, network_mysqld_lenenc_type *type) {
 ...
 * @packet:   the MySQL-packet to decode
 * @v:        destination of the integer
 *
...
/**
 * network_mysqld_proto_skip:
 * @packet: the MySQL network packet
 * @size:   bytes to skip
 *
 *
...
/**
 * network_mysqld_proto_peek_int_len:
 * @packet: the MySQL network packet
 * @v:      destination of the integer
 * @size:   byte-len of the integer to decode
 *
 ...
 * @v: (out): destination of the decoded integer
 * @size: size 
 *
 * decode a integer of byte-len @size and store it in @v. 
 *
 * moves the offset of @packet
 *
...
/**
 * network_mysqld_proto_get_int8:
 * @packet: the MySQL network packet
 * @v:      dest for the number
 *
...
/**
 * network_mysqld_proto_peek_int8:
 * @packet: the MySQL network packet
 * @v:      dest for the number
 *
 * get a 8-bit integer from the network packet
...
/**
 * network_mysqld_proto_peek_int16:
 * @packet: the MySQL network packet
 * @v:      dest for the number
 *
 * get a 16-bit integer from the network packet
...
/**
 * network_mysqld_proto_get_int24:
 * @packet: the MySQL network packet
 * @v:      dest for the number
 *
...
/**
 * network_mysqld_proto_peek_int32:
 * @packet: the MySQL network packet
 * @v:      dest for the number
 *
 ...
 */
int
network_mysqld_proto_peek_int32(network_packet *packet, guint32 *v) {
	guint64 v64;
 ...
 */
int
network_mysqld_proto_get_int48(network_packet *packet, guint64 *v) {
	guint64 v64;
 ...
 */
int
network_mysqld_proto_get_int64(network_packet *packet, guint64 *v) {
 ...
 * @packet: the MySQL network packet
 * @c:      character to find
 * @pos:    offset into the packet the 'c' was found
 *
 * find a 8-bit integer in the network packet
 *
 * Returns: a the decoded integer
 * See: network_mysqld_proto_get_int_len()
 */
int
network_mysqld_proto_find_int8(network_packet *packet, guint8 c, guint *pos) {
	int err = 0;
	guint off = packet->offset;
...
/**
 * network_mysqld_proto_get_string_len:
 * @packet: the MySQL network packet
 * @s:      dest of the string
 * @len:    bytes to read from the packet
 *
 * get a string from the network packet
 *
 * if @len is 0, *@s will be %NULL
 * otherwise *@s will contain a NUL-terminate string
 *
 * Returns: 0 on success, non-0 otherwise
 */
int
network_mysqld_proto_get_string_len(network_packet *packet, gchar **s, gsize len) {
	gchar *str;
	...
	if (packet->offset + len > packet->data->len) {
		g_critical("%s: packet-offset out of range: %u + %"G_GSIZE_FORMAT" > %"G_GSIZE_FORMAT, 
				G_STRLOC,
 ...
 * @packet: the MySQL network packet
 * @s: (out): destination of the decoded string
 * @_len: (allow-none): destination of the length of the decoded string, if len is non-NULL
 *
 ...
 */
int
network_mysqld_proto_get_lenenc_string(network_packet *packet, gchar **s, guint64 *_len) {
	guint64 len;
...
/**
 * network_mysqld_proto_get_string:
 * @packet: the MySQL network packet
 * @s:      dest of the string
 *
 ...
 * @packet: the MySQL network packet
 * @len:    bytes to copy
 * @out:    a GString which carries the string
 *
 ...
 */
int
network_mysqld_proto_get_gstring_len(network_packet *packet, gsize len, GString *out) {
	int err = 0;
 ...
 *
 * Returns:       a pointer to the string in out
 *
 * See: network_mysqld_proto_get_gstring_len()
 */
int
network_mysqld_proto_get_gstring(network_packet *packet, GString *out) {
	guint64 len;
	int err = 0;
 ...
 */
int
network_mysqld_proto_get_lenenc_gstring(network_packet *packet, GString *out) {
	guint64 len;
	int err = 0;
 ...
 *
 * Returns: a empty array of MYSQL_FIELD
 */
GPtrArray *
 ...
 * @fields: (element-type MYSQL_FIELD): array of MYSQL_FIELD to free
 *
 * free a array of MYSQL_FIELD 
 *
 * See: network_mysqld_proto_fielddef_free()
 */
void
...
/**
 * network_mysqld_proto_set_packet_len:
 * @_header:  string of at least 4 byte to write the packet header to
...
/**
 * network_mysqld_proto_set_packet_id:
 * @_header: network packet
 * @id: id
 *
 * set the packet id
 *
 * @_header: has to be at least 3 bytes long
 *
 * Returns: 0
 */
int
 ...
 * @id: packet id
 *
 * append the packet id to the @_header
 */
int
...
/**
 * network_mysqld_proto_get_packet_len:
 * @_header: the first 3 bytes of the network packet
 *
 * decode the packet length from a packet header
 *
 * Returns: the packet length
 * See: network_mysqld_proto_set_packet_len()
 */
guint32
 ...
 */
int
network_mysqld_proto_append_lenenc_string_len(GString *packet, const char *s, guint64 length) {
	if (!s) {
 ...
 */
int
network_mysqld_proto_append_lenenc_string(GString *packet, const char *s) {
 ...
 */
int
network_mysqld_proto_append_int48(GString *packet, guint64 num) {
 ...
 */
int
network_mysqld_proto_append_int64(GString *packet, guint64 num) {
 ...
 * @response: destination of the hashed password
 ...
 *   SHA1( password )
 *
 * See: network_mysqld_proto_password_scramble()
 */
int
...
/**
 * network_mysqld_proto_password_scramble:
 * @response:         dest 
 * @challenge:        the challenge string as sent by the mysql-server
 * @challenge_len:    length of the challenge
 * @hashed_password:  hashed password
 * @hashed_password_len: length of the hashed password
 *
 ...
 * @hashed_password:  dest of the hashed password
 * @challenge:        the challenge string as sent by the mysql-server
 * @challenge_len:    length of the challenge
 * @response:         auth response as sent by the client 
 * @response_len:     length of response
 * @double_hashed:    the double hashed password as stored in the mysql.server table (without * and unhexed)
 * @double_hashed_len: length of double_hashed
 *
...
/**
 * network_mysqld_proto_skip_network_header:
 * @packet: network packet
 *
 * skip the mysql header in the packet
 *
