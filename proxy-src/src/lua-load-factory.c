...
/**
 * SECTION:lua-load-factory
 * @short_description: a load factory for Lua scripts
 *
 * we want to cache the scripts, but reusing the scripts would also mean 
 * we reuse the functions environment which would destroy a elegant way 
 * of creating a local variable scope for the script.
 *
 * Instead if we create anonymous function which returns the function 
 * we are interested in. That anonymous function will have the env we 
 * are interested in.
 */
 ...
 * @s: lua code
 *
 * wrap lua code in "return function()" .. "end\n" to be able to assign it a new fenv each time
 */
int
luaL_loadstring_factory(lua_State *L, const char *s) {
 ...
 */
int
luaL_loadfile_factory(lua_State *L, const char *filename) {
	int ret;
	load_factory_t factory;
