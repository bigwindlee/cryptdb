...
/**
 * SECTION:lua-scope
 * @short_description: a global scope for Lua
 *
 * a shared global scope for Lua protected by mutexes
 */
...
#include "chassis-stats-private.h"
 ...
 * create a #lua_scope
 *
 * Deprecated: 0.7.0: use lua_scope_new() instead
 */
lua_scope *
lua_scope_init(void) {
 ...
 * @pos: source location
 *
 * take control over the @sc
 */
void
lua_scope_get(lua_scope *sc, const char G_GNUC_UNUSED* pos) {
 ...
 * @pos: source location
 *
 * release control on the lua-scope
 */
void
lua_scope_release(lua_scope *sc, const char* pos) {
#ifdef HAVE_LUA_H
 ...
 */
lua_State *
lua_scope_load_script(lua_scope *sc, const gchar *name) {
	lua_State *L = sc->L;
 ...
 */
void
proxy_lua_dumptable(lua_State *L) {
	g_assert(lua_istable(L, -1));
 ...
 */
void
proxy_lua_dumpstack(lua_State *L) {
	int i;
...
/**
 * proxy_lua_dumpstack_verbose:
 * @L: a lua_State
 *
 * perform a verbose dump of the lua stack
 */
void
proxy_lua_dumpstack_verbose(lua_State *L) {
    int i;
    int top = lua_gettop(L);
    GString *stack_desc;
    if (top == 0) {
 ...
 */
void
proxy_lua_currentline(lua_State *L, int level) {
	lua_Debug ar;
	const char *name;
...
/*
 * chassis_lua_alloc:
 * @userdata: %NULL and unused in our case (userdata passed to lua_newstate)
 * @ptr: the pointer to the block to be malloced/realloced/freed
 ...
 */
static void *
chassis_lua_alloc(void G_GNUC_UNUSED *userdata, void *ptr, size_t osize, size_t nsize) {
	gpointer p;
	gint cur_size;
