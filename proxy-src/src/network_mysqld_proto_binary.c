...
/**
 * SECTION:network_mysqld_proto_binary
 * @short_description: codec's for the binary MySQL client protocol
 *
 * Encoders and Decoders for the MySQL client/server protocol, the binary parts
 */
...
/* ints */
static int
network_mysqld_proto_binary_get_int_type(network_packet *packet, network_mysqld_type_t *type) {
	int err = 0;
	guint8 i8;
	guint16 i16;
	guint32 i32;
...
static int
network_mysqld_proto_binary_append_int_type(GString *packet, network_mysqld_type_t *type) {
	guint64 i64;
	guint8  i8;
	guint16 i16;
	guint32 i32;
...
/* double */
static int
network_mysqld_proto_binary_get_double_type(network_packet *packet, network_mysqld_type_t *type) {
	int err = 0;
	union {
		double d;
...
static int
network_mysqld_proto_binary_append_double_type(GString *packet, network_mysqld_type_t *type) {
	union {
		double d;
		guint8 d_char_shadow[sizeof(double)];
	} double_copy;
...
/* float */
static int
network_mysqld_proto_binary_get_float_type(network_packet *packet, network_mysqld_type_t *type) {
	int err = 0;
	union {
		float f;
...
static int
network_mysqld_proto_binary_append_float_type(GString *packet, network_mysqld_type_t *type) {
	union {
		float f;
		guint8 f_char_shadow[sizeof(float)];
	} float_copy;
...
static int
network_mysqld_proto_binary_get_string_type(network_packet *packet, network_mysqld_type_t *type) {
	GString *str;
	int err = 0;
...
static int
network_mysqld_proto_binary_append_string_type(GString *packet, network_mysqld_type_t *type) {
	const char *s;
	gsize s_len;
	int err = 0;
 ...
 */
static int
network_mysqld_proto_binary_get_date_type(network_packet *packet, network_mysqld_type_t *type) {
	int err = 0;
	guint8 len;
...
static int
network_mysqld_proto_binary_append_date_type(GString G_GNUC_UNUSED *packet, network_mysqld_type_t G_GNUC_UNUSED *type) {
	/* not implemented yet */
	return -1;
}
 ...
 */
static int
network_mysqld_proto_binary_get_time_type(network_packet *packet, network_mysqld_type_t *type) {
	int err = 0;
	guint8 len;
...
static int
network_mysqld_proto_binary_append_time_type(GString G_GNUC_UNUSED *packet, network_mysqld_type_t G_GNUC_UNUSED *type) {
	/* not implemented yet */
	return -1;
}

/**
 * network_mysql_proto_binary_type_is_valid_input:
 * @field_type: field type
 *
 * valid types for prepared statements parameters we receive from the client
 *
 * Returns: %TRUE if the type can be accepted, %FALSE if not
 */
gboolean
network_mysql_proto_binary_type_is_valid_input(enum enum_field_types field_type) {
	switch (field_type) {
	case MYSQL_TYPE_TINY:
	case MYSQL_TYPE_SHORT:
	case MYSQL_TYPE_LONG:
 ...
 *
 * Returns: %TRUE if the type can be generated, %FALSE if not
 */
gboolean
network_mysql_proto_binary_is_valid_output(enum enum_field_types field_type) {
	switch (field_type) {
	case MYSQL_TYPE_TINY:
	case MYSQL_TYPE_SHORT:
	case MYSQL_TYPE_INT24:
...
/**
 * network_mysqld_proto_binary_get_type:
 * @packet: a network packet
 * @type: type
 *
 * get a #network_mysqld_type_t from @packet
 *
 * Returns: -1 on error, 0 on success
 */
int
network_mysqld_proto_binary_get_type(network_packet *packet, network_mysqld_type_t *type) {
	switch (type->type) {
	case MYSQL_TYPE_TINY:
	case MYSQL_TYPE_SHORT:
	case MYSQL_TYPE_LONG:
...
/**
 * network_mysqld_proto_binary_append_type:
 * @packet: a network packet
 * @type: type
 *
 * append a #network_mysqld_type_t to @packet
 *
 * Returns: -1 on error, 0 on success
 */
int
network_mysqld_proto_binary_append_type(GString *packet, network_mysqld_type_t *type) {
	switch (type->type) {
	case MYSQL_TYPE_TINY:
	case MYSQL_TYPE_SHORT:
	case MYSQL_TYPE_LONG:
