...
/**
 * SECTION:network-mysqld
 * @short_description: mysql connection handling 
 *
 * life-cycle handling
 *
 */
...
/**
 * network_mysqld_priv_init:
 *
 * create a new chassis_private
 */
chassis_private *
...
/**
 * network_mysqld_priv_shutdown:
 * @chas: the chassis
 * @priv: the private data
 *
 * close all connections currently open
 */
void
...
/**
 * network_mysqld_priv_free:
 * @chas: unused chassis
 * @priv: private data
 *
 * free the chassis_private
 */
void
 ...
 * @srv: the chassis
 *
 * init the private data in the chassis
 */
int
...
/**
 * network_mysqld_con_init:
 *
 * create a #network_mysqld_con
 *
 * Deprecated: 0.8.0: use network_mysqld_con_new()
 * Returns: a #network_mysqld_con
 */
network_mysqld_con *
network_mysqld_con_init() {
...
/**
 * network_mysqld_add_connection:
 * @srv: the chassis
 * @con: the connection
 *
 * register a connection in the chassis to allow closing of all connections at shutdown
 */
void
network_mysqld_add_connection(chassis *srv, network_mysqld_con *con) {
	con->srv = srv;
 ...
 */
int
network_mysqld_queue_sync(network_socket *dst, network_socket *src) {
 ...
 */
int
network_mysqld_queue_append(network_socket *sock, network_queue *queue, const char *data, size_t data_len) {
	gsize packet_offset = 0;

	do {
		GString *s;
		gsize cur_data_len = MIN(data_len, PACKET_LEN_MAX);

		s = g_string_sized_new(data_len + 4);
		...
		network_mysqld_proto_append_packet_len(s, cur_data_len);
		network_mysqld_proto_append_packet_id(s, ++sock->last_packet_id);
		g_string_append_len(s, data + packet_offset, cur_data_len);

		network_queue_append(queue, s);

		if (data_len == PACKET_LEN_MAX) {
...
/**
 * network_mysqld_con_send_ok_full:
 * @con:             a client socket 
 * @affected_rows:   affected rows 
 * @insert_id:       insert_id 
 * @server_status:   server_status (bitfield of SERVER_STATUS_*) 
 * @warnings:        number of warnings to fetch with SHOW WARNINGS 
 *
 ...
 */
int
network_mysqld_con_send_ok_full(network_socket *con, guint64 affected_rows, guint64 insert_id, guint16 server_status, guint16 warnings ) {
 ...
 * - no warnings
 *
 * See: network_mysqld_con_send_ok_full()
 *
 */
int
 ...
 * @con:         the client connection
 * @errmsg:      the error message
 * @errmsg_len:  byte-len of the error-message
 * @errorcode:   mysql error-code we want to send
 * @sqlstate:    if none-NULL, 5-char SQL state to send, if NULL, default SQL state is used
 *
 ...
 */
int
network_mysqld_con_send_error_full(network_socket *con, const char *errmsg, gsize errmsg_len, guint errorcode, const gchar *sqlstate) {
...
/**
 * network_mysqld_con_send_error_pre41_full:
 * @con:         the client connection
 * @errmsg:      the error message
 * @errmsg_len:  byte-len of the error-message
 * @errorcode:   mysql error-code we want to send
...
network_mysqld_con_get_packet(chassis G_GNUC_UNUSED*chas, network_socket *con) {
	GString *packet = NULL;
	GString header;
 ...
 */
network_socket_retval_t
network_mysqld_read(chassis G_GNUC_UNUSED*chas, network_socket *con) {
...
/**
 * network_mysqld_write:
 * @chas: chassis
 * @con: socket
 *
 * write content of the send queue
 *
 * See: network_socket_write()
 */
network_socket_retval_t
network_mysqld_write(chassis G_GNUC_UNUSED*chas, network_socket *con) {
...
/**
 * plugin_call:
 * @srv:      the global context
 * @con:      the connection context
...
/**
 * network_mysqld_con_reset_command_response_state:
 * @con: socket
 *
...
/**
 * network_mysqld_con_state_get_name:
 * @state: a state
 *
 * get the name of a connection state
 */
const char *
network_mysqld_con_state_get_name(network_mysqld_con_state_t state) {
	switch (state) {
...
static int
network_mysqld_con_track_auth_result_state(network_mysqld_con *con) {
	network_packet packet;
	guint8 state;
	int err = 0;
 ...
 * @event_fd:     fd on which the event was fired
 * @events:       the event that was fired
 * @user_data:    the connection handle
 *
 ...
 * @event_fd:     fd on which the event was fired
 * @events:       the event that was fired
 * @user_data:    the listening connection handle
 *
 * accept a connection
 *
...
/**
 * network_mysqld_con_send_resultset:
 * @con: a socket
 * @fields: (element-type MYSQL_FIELD): 
 * @rows: (element-type GPtrArray):
 *
 * @todo move to network_mysqld_proto
 */
int
