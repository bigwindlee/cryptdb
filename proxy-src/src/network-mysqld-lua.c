...
/**
 * SECTION:network-mysqld-lua
 * @short_description: Lua wrapper for network-mysqld
 *
 * wrappers around the functions of network-mysqld
 */

/**
 * network_mysqld_con_lua_new:
 *
 * create a #network_mysqld_con_lua_t
 *
 * Returns: a #network_mysqld_con_lua_t
 */
 ...
 * @st: a #network_mysqld_con_lua_t
 *
 * free a #network_mysqld_con_lua_t
 */
void
network_mysqld_con_lua_free(network_mysqld_con_lua_t *st) {
	if (!st) return;
 ...
 */
static int
proxy_connection_get(lua_State *L) {
 ...
 */
static int
proxy_connection_set(lua_State *L) {
	network_mysqld_con *con = *(network_mysqld_con **)luaL_checkself(L);
	network_mysqld_con_lua_t *st;
	gsize keysize = 0;
	const char *key = luaL_checklstring(L, 2, &keysize);
 ...
 * @L: lua_State
 *
 * push metatable for #network_mysqld_con on the stack
 * 
 * Returns: number of items added to the stack
 */
int
network_mysqld_con_getmetatable(lua_State *L) {
 ...
 */
void
network_mysqld_lua_setup_global(lua_State *L , chassis_private *g) {
 ...
 */
int
network_mysqld_lua_load_script(lua_scope *sc, const char *lua_script) {
 ...
 * @con: connection
 * @lua_script: name of the lua script
 *
...
network_mysqld_con_lua_register_callback(network_mysqld_con *con, const char *lua_script) {
	lua_State *L = NULL;
...
/**
 * network_mysqld_lua_init_global_fenv:
 * @L: lua_State
 *
 * init the global proxy object 
 */
void
network_mysqld_lua_init_global_fenv(lua_State *L) {
...
/**
 * network_mysqld_con_lua_handle_proxy_response:
 * @con: a connection
 ...
 */
int
network_mysqld_con_lua_handle_proxy_response(network_mysqld_con *con, const gchar *lua_script) {
	network_mysqld_con_lua_t *st = con->plugin_con_state;
	int resp_type = 1;
	const char *str;
	size_t str_len;
