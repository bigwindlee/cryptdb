...
/**
 * SECTION:chassis-filemode
 * @short_description: functions around filemodes
 *
 * file-mode checks
 */
 ...
 *
 * check the mode of a file against #CHASSIS_FILEMODE_SECURE_MASK
 *
 * Deprecated: 0.8.1: use chassis_filemode_check_full() instead
 */
int
 ...
 * @filename: name of the file to check
 * @required_filemask: file-mask to check for
 ...
 *
 * Returns: 0 on success, -1 of the file cannot be accessed or is the wrong
 * type of file, and 1 if permissions are wrong. As Windows has no concept
 * of owner/group/other, this function just return 0 for windows
 *
 * FIXME? this function currently ignores ACLs
 */
int
chassis_filemode_check_full(const gchar *filename, int required_filemask, GError **gerr) {
#ifndef _WIN32
	struct stat stbuf;
	mode_t		fmode;
