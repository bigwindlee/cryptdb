...
#ifdef _WIN32
/**
 * SECTION: chassis-win32-service
 * @short_description: win32 service functions
 *
 * win32 functions for
 * - service interface
 * - inval parameter handling
 *
 */
 ...
 * @pReserved: reserved
 *
 * invalid parameter handler for win32
 *
 * by default win32 runtime trigger a abort() if a invalid param is received for some of the values
 * instead of returning EINVAL
 *
 * See: chassis_win32_invalid_parameter_handler_set()
 */
void
chassis_win32_invalid_parameter_handler_ignore(
 ...
 * invalid parameter handler for win32
 *
 * log the location of the invalid parameter
 *
 * See: chassis_win32_invalid_parameter_handler_set()
 */
void
...
/**
 * chassis_win32_invalid_parameter_handler_set:
 * @new_handler: a new handler
 *
 * set the @new_handler
 *
 * Returns: the old handler
 * See: chassis_win32_invalid_parameter_handler_log(), chassis_win32_invalid_parameter_handler_ignore()
 */
...
/**
 * chassis_win32_is_service:
 *
 * check if we are running as win32 service
 *
 * Returns: %TRUE if running as service, %FALSE otherwise
 */
gboolean
chassis_win32_is_service(void) {
	return chassis_win32_running_as_service;
}

/**
 * chassis_win32_service_set_state:
 * @new_state: state to set
 * @wait_msec: timeout in milli seconds
 *
 * set the win32 service state
 */
void
chassis_win32_service_set_state(DWORD new_state, int wait_msec) {
	DWORD status;
 ...
 */
static DWORD WINAPI
chassis_win32_service_ctrl(DWORD Opcode, DWORD EventType, LPVOID EventData, LPVOID _udata) {
	switch(Opcode) {
 ...
 */
static void WINAPI
chassis_win32_service_start(DWORD argc, LPTSTR *argv) {
 ...
 * @argc: argc
 * @argv: argv
 * @_main_cmdline: function to call if we are not a service
 *
 * Determine whether we are called as a service and set that up.
 * Then call main_cmdline to do the real work.
 */
int
main_win32(int argc, char **argv, int (*_main_cmdline)(int, char **)) {
	WSADATA wsaData;
