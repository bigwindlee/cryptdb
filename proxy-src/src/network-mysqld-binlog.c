...
/**
 * SECTION:network-mysqld-binlog
 * @short_description: binlog reader/writer
 *
 * readers and writes for the MySQL binlog
 */
 ...
 * @i: value
 *
 * create space for a guint64
 *
 * Returns: a guint64* pointing to @i
 */
guint64 *
guint64_new(guint64 i) {
	guint64 *ip;
...
/**
 * network_mysqld_binlog_new:
 *
 * create a #network_mysqld_binlog
 *
 * Returns: a #network_mysqld_binlog
 */
 ...
 * @binlog: a #network_mysqld_binlog
 *
 * free a #network_mysqld_binlog
 */
void
network_mysqld_binlog_free(network_mysqld_binlog *binlog) {
	if (!binlog) return;
...
/**
 * network_mysqld_binlog_open:
 * @binlog: a #network_mysqld_binlog
 * @filename: name of the binlog file to open
 * @mode: 'r' or 'w'
 *
 * open a binlog-file for reading or writing 
 */
int
network_mysqld_binlog_open(network_mysqld_binlog *binlog, const char *filename, const char *mode_str) {
...
/**
 * network_mysqld_binlog_append:
 * @binlog: a #network_mysqld_binlog
 * @event: a event
 *
 * append a event to a binlog
 */
int
network_mysqld_binlog_append(network_mysqld_binlog *binlog, network_mysqld_binlog_event *event) {
...
/**
 * network_mysqld_binlog_read_event_header:
 * @binlog: a open binlog file
 * @packet: packet
 *
 * read the header of a binlog event from the current file position
 */
int
network_mysqld_binlog_read_event_header(network_mysqld_binlog *binlog, network_packet *packet) {
...
/**
 * network_mysqld_binlog_read_event:
 * @binlog: a open binlog file
 * @packet: packet
...
/**
 * network_mysqld_binlog_event_new:
 *
 * create a new binlog event
 *
 * Returns: a #network_mysqld_binlog_event
 */
 ...
 * @packet: packet
 *
 * read 1-byte binlog status from a binlog file
 *
 * Returns: 0 if binlog is ok, -1 on error
 */
int
network_mysqld_proto_get_binlog_status(network_packet *packet) {
	guint8 ok;
 ...
 * @event: a event struct
 *
 * get event header from a @packet
 *
 * Returns: 0 on success, non zero otherwise
 */
int
network_mysqld_proto_get_binlog_event_header(network_packet *packet, network_mysqld_binlog_event *event) {
	int err = 0;
	guint8 event_type;
...
/**
 * network_mysqld_proto_append_binlog_event_header:
 * @packet: packet
 * @event: event
 *
 * append event header to a @packet
 */
int
network_mysqld_proto_append_binlog_event_header(GString *packet, network_mysqld_binlog_event *event) {
...
/**
 * network_mysqld_proto_append_binlog_event:
 * @packet: packet
 * @event: event
 *
 * append the event body to the @packet
 */
int
network_mysqld_proto_append_binlog_event(GString *packet, network_mysqld_binlog_event *event) {
	guint i;
...
/**
 * network_mysqld_proto_get_binlog_event:
 * @packet: packet
 * @binlog: binlog
 * @event: event
 *
 * get the event body from the binlog @packet
 */
int network_mysqld_proto_get_binlog_event(network_packet *packet, 
	...
	case FORMAT_DESCRIPTION_EVENT:
		...
		if ((maj << 16) + (min << 8) + (pat << 0) >= 0x050602) {
			...
			g_assert_cmpint(event->event.format_event.event_header_sizes_len, ==, ENUM_END_EVENT - 1 + 1 + 4);

			event->event.format_event.event_header_sizes_len -= 1 + 4; /* FIXME: check how it is implemented without CRC32 */
		} else {
			event->event.format_event.event_header_sizes_len = packet->data->len - packet->offset;
		}
 ...
 */
void
network_mysqld_binlog_event_free(network_mysqld_binlog_event *event) {
	if (!event) return;
...
/**
 * network_mysqld_binlog_event_get_name:
 * @event: event
 *
 ...
 */
const char *
network_mysqld_binlog_event_get_name(network_mysqld_binlog_event *event) {
...
/**
 * network_mysqld_binlog_event_get_id:
...
/**
 * network_mysqld_binlog_dump_new:
 *
 * create a #network_mysqld_binlog_dump
 *
 * Returns: a #network_mysqld_binlog_dump
 */
 ...
 * @dump: a #network_mysqld_binlog_dump
 *
 * append a #network_mysqld_binlog_dump on @packet
 *
 * Returns: 0 on success, -1 on error
 */
int
network_mysqld_proto_append_binlog_dump(GString *packet, network_mysqld_binlog_dump *dump) {
 ...
 * @dump: a #network_mysqld_binlog_dump
 *
 * free a #network_mysqld_binlog_dump
 */
void
network_mysqld_binlog_dump_free(network_mysqld_binlog_dump *dump) {
	if (!dump) return;
...
/**
 * network_mysqld_binlog_event_tablemap_get_type_len:
 * @type: type
 *
...
/** 
 * network_mysqld_binlog_event_tablemap_from_table_columns:
 * @event: event
 * @columns: columns
 *
...
/**
 * network_mysqld_binlog_event_tablemap_to_table_columns:
 * @event: event
 * @columns: columns
 *
...
/**
 * network_mysqld_binlog_event_tablemap_to_table:
 * @event: event
 * @tbl: table
 *
...
/**
 * network_mysqld_binlog_event_tablemap_from_table:
 * @event: event
 * @tbl: table
 *
