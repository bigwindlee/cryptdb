...
/**
 * SECTION:chassis-stats
 * @short_description: global statistics of the chassis
 *
 * |[
 * chassis_stats_t *stats;
 *
 * stats = chassis_stats_new();
 *
 * chassis_stats_free(stats);
 * ]|
 *
 *
 */
 ...
 * chassis_stats_new:
 *
 * init the global chassis_global_stats
 *
 * multiple calls return the same #chassis_stats_t
 *
 * Returns: the global #chassis_stats_t
 */
chassis_stats_t *
chassis_stats_new(void) {
...
/**
 * chassis_stats_free:
 * @stats: a #chassis_stats_t
 *
 * free the global stats
 *
 * should only be called once
 */
void
chassis_stats_free(chassis_stats_t *stats) {
	if (!stats) return;
 ...
 * @stats: a #chassis_stats_t
 *
 * get a hash(str, int) of all stat values
 *
 * Returns: (element-type gchar int): a hash
 */
GHashTable*
chassis_stats_get(chassis_stats_t *stats){
