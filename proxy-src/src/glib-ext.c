...
/**
 * SECTION:glib-ext
 * @short_description: helper functions for common glib operations
 *
 * - typesafe functions for GDestroyNotify and friends
 * - functions that should be part of glib like g_string_assign_len()
 */
 ...
 */
void
g_list_string_free(gpointer data, gpointer G_GNUC_UNUSED user_data) {
	g_string_free(data, TRUE);
}

/**
 * g_hash_table_string_free:
 * @data: (type GString): a #GString
 *
 ...
 *
 * Returns: TRUE if the two #GString are equal, FALSE otherwise
 */
gboolean
 ...
 */
gboolean
g_hash_table_true(gpointer G_GNUC_UNUSED key, gpointer G_GNUC_UNUSED value, gpointer G_GNUC_UNUSED u) {
	return TRUE;
}	
 ...
 *
 * wrapper around #GHashTable that have #GString as key to use static strings as lookup key
 */
gpointer
g_hash_table_lookup_const(GHashTable *h, const gchar *name, gsize name_len) {
	GString key;
 ...
 *
 * if the length of a string is known (static strings, #GString, ...) strleq() faster than strcmp()
 * if you only want to test of equality
 *
 * Returns: %TRUE if size and value are equal, %FALSE otherwise
 */
gboolean
...
/**
 * g_string_get_time:
 * @s: destination #GString
 * @gt: a UTC #GTimeVal
 *
 * convert a #GTimeVal into a UTC ISO8601-string
 *
 * should be deprecated in favour of
 * - return @s to use the function inline and as the current return value is never != 0
 * - function name doesn't say that it returns a ISO8601 string
 * - function name doesn't say that it assumes the timestamp is UTC
 * 
 * Returns: 0 on success
 */
int
g_string_get_time(GString *s, GTimeVal *gt) {
	time_t t = gt->tv_sec;
...
/**
 * g_string_get_current_time:
 * @s: a string
 *
 * set the current timestamp to @s in ISO8601 notation
 *
 * Returns: 0 on success
 * See: g_string_get_time()
 */
int
g_string_get_current_time(GString *s) {
	GTimeVal gt;
...
/**
 * g_debug_hexdump:
 * @msg: log-string prefix
 * @_s: memory buffer
 * @len: length of @_s
 *
 * dump a memory block as hexdump using g_debug()
 */
void
 ...
 *
 * Returns: %TRUE if @a and @b are equal using a case-insensitve UTF8 compare, %FALSE otherwise
 */
gboolean
g_string_equal_ci(const GString *a, const GString *b) {
	char *a_ci, *b_ci;
 ...
 *
 * See: strleq()
 * Returns: %TRUE if size and value of the input are equal, %FALSE otherwise
 */
gboolean
g_memeq(const char *a, gsize a_len, const char *b, gsize b_len) {
	if (a_len != b_len) return FALSE;
